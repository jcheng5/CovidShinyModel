...
runMarkov <- function(new.g.vec, trans.mat){
  ...
  colnames(df.final) <- c('g', 'icu', 'vent', 'disc', 'death')
  return(df.final)
}
...
SIR <- function(S0, I0, R0, beta.vector, gamma.r, gamma.h, 
  ...
  S <- IH <- IR <- R <- newG <- rep(NA_real_, num.days)
  S[1] <- S0
  ...
  df.return <- data.frame(days = 1:num.days, S, I, IR, IH, R, newG)
...
find.curr.estimates = function(S0, beta.vector, gamma.r, gamma.h, hosp.rate, trans.mat,
  ...
  SIR.df$diff_proj <- abs(SIR.df$hosp.tot - num.actual)
